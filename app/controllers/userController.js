    ...
    let validateUserInput = () => {
        return new Promise((resolve, reject) => {
            ...
            if (!check.isEmpty(req.body.email) && !check.isEmpty(req.body.firstName) && !check.isEmpty(req.body.lastName)
                && !check.isEmpty(req.body.mobileNumber) && !check.isEmpty(req.body.password) && !check.isEmpty(req.body.country)) {
    ...
    let sendMail = (newUserObj) => {
        return new Promise((resolve, reject) => {
            ...
            const mailOptions = {
                ...
                html:'<a href="http://edlist.webdeveloperjourney.xyz/verify/'+data.hash+'">http://edlist.webdeveloperjourney.xyz/'+data.hash+'</a>'
               };
    ...
    let findUser = () => {
        ...
        return new Promise((resolve, reject) => {
            if (req.body.email) {
                ...
                let apiResponse = response.generateResponse(true, '"Email" parameter is missing', 400, null)
                reject(apiResponse)
            }
        });
    }   //find user end
...
let forgotPassword=(req,res)=>{
   ...
   const mailOptions = {
    ...
    html: "<p>Hi!,<br/>Please <a href='http://edlist.webdeveloperjourney.xyz/reset/"+emailEncrypt+"'>Click here</a> to change your password!</p>"
